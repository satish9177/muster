...
The verified Stage-90 execution acquired employer and site evidence; it did not
run all three agents. It replays Ravi's unsigned, inert text claim rather than
driving the Worker Agent, because a claim is not something a source can be
asked for and no Worker Agent was deployed. The committed Worker Agent ADK path
does include Gemini interpretation, but it was not rerun in execution
`muster-control-plane-hero-htkpt`. The cloud run prints predicate names,
...
In the verified Stage-90 acquisition, the Employer Agent received a
