...
function boundaryEvent(artifact: CaseTraceArtifact): RawTraceEvent {
  ...
  return event({
    ...
    summary: "The identity holding the case record cannot read private site material.",
    ...
    inspector: {
      ...
      key_id: null,
      authority_grant: `No ${boundary.operation} grant on ${boundary.target_class}`,
      predicates: [`${boundary.operation}(${boundary.target_class}) = ${boundary.result}`],
      disclosure: "None. The request was refused before raw material could cross the boundary.",
...
function actionEvent(
  ...
  return event({
    ...
    summary: "The proposal is ready for a future authorization gate. Nothing has been sent.",
    result: artifact.result.action.execution.status.replace("_", " "),
    result_tone: "pending",
    tags: ["ACTION GATE PENDING"],
    inspector: {
      source_class: "DETERMINISTIC CASE RESULT",
      source_identity: "MUSTER Control Plane",
      key_id: null,
      authority_grant: "None to execute — Action Gate is not implemented",
      ...
      provenance_note: "The action state is structured as NOT_EXECUTED; the UI does not infer it.",
    },
  });
}
