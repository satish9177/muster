...
A deliberate second copy of ``agent_tests/support/shell.py``.  The two suites
are rooted separately -- the reason is in ``support/paths.py`` -- and neither
can import the other's ``support`` package when it is run on its own, so a
resolver living in only one of them would leave the other with exactly the
failure below.
